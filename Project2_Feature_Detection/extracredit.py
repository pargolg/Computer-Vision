...
def gaussian_blur_kernel_2d(sigma, height, width):
        ...
        for i in range(height):
            for j in range(width):
                x = j-halfW
                y = i-halfH
                tmp = np.exp(-1*(x*x+y*y)/(2.0*sigma*sigma))/(2.0*np.pi*sigma*sigma)
                sum = sum + tmp
                out[i,j] = tmp
        return out/sum 
...
class KeypointDetector(object):   
    ...
    def detectKeypoints(self, image):
        ...
        return hkd.detectKeypoints(image)
...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # TODO-BLOCK-BEGIN
        weights = gaussian_blur_kernel_2d(0.5, 5, 5)
            ...
            for col in range(srcImage.shape[1]):
                ...
                for i in range(5):
                    for j in range(5):
                        ...
                        curRow = row - 2 + i
                        curCol = col - 2 + j
                        if (inbounds(srcImage.shape, (curRow, curCol))):
                            ix = Ix[curRow, curCol]
                            iy = Iy[curRow, curCol]
                        A += w * ix * ix
                        B += w * ix * iy
                ...
                harrisImage[row,col] = strength
                angle = np.degrees(np.arctan2(Iy[row,col], Ix[row,col]))
                orientationImage[row,col] = angle
        # TODO-BLOCK-END
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOCK-BEGIN
        window = 11
        for row in range(harrisImage.shape[0]):
            for col in range(harrisImage.shape[1]):
                out = True
                val = harrisImage[row,col]
                for i in range(window):
                    for j in range(window):
                        curRow = row - (window-1)/2 + i
                        curCol = col - (window-1)/2 + j
                        if (not out) or (not inbounds(harrisImage.shape, (curRow, curCol))) or (curRow == row and curCol == col):
                            continue
                        if harrisImage[curRow,curCol] > val:
                            out = False
                        if (not out):
                            break
                    if (not out):
                        break
                destImage[row,col] = out
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for pointNum, f in enumerate(keypoints):
            x, y = f.pt
            x, y = int(x), int(y)
            ...
            for i in range(5):
                for j in range(5):
                    curRow = y - 2 + i
                    curCol = x - 2 + j
                    if inbounds(grayImage.shape, (curRow, curCol)):
                        desc[pointNum, index] = grayImage[curRow, curCol]
                    index += 1
            # TODO-BLOCK-END
